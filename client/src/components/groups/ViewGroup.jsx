...
function ViewGroup() {
    ...
    const { updateGroup, isCreating, getGroup, group, isGroupLoading, isAdmin } = useGroupStore();
    const [imagePreview, setImagePreview] = useState(group ? group.profilePic : null);
    ...
    useEffect(() => {
        if (group) {
            const filteredMember = group.members?.filter((member) => authUser._id?.toString() !== member._id.toString());
            setName(group.name || "");
            setDiscription(group.description || "");
            setMembers(filteredMember || []);
            setImagePreview(group.profilePic || null);
        }
    }, [group]);
        ...
        <div className='px-4 py-6 relative'>
            ...
            <div className='space-y-4'>
                ...
                <button onClick={handleSubmit} className='btn btn-md mt-6'
                    disabled={isCreating}>
                    {isCreating ? "Group is updating..." : "Update Group"}
                </button>
            </div>
